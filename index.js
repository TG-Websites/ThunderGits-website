...
async function fetchJobs() {
    try {
        ...
        jobs.forEach((job) => {
            ...
            const card = `
        <div class="group bg-white/[0.03] border border-white/10 rounded-3xl p-8 hover:border-primary/30 transition-all duration-500 hover:-translate-y-2 flex flex-col" data-aos="fade-up" data-aos-delay="100">
          <div class="flex items-center gap-4 ">
            <div class="w-12 h-12 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center group-hover:bg-primary transition-colors">
              <i class="ri-palette-line text-2xl text-primary group-hover:text-white transition-colors"></i>
            </div>
            <h3 class="text-xl font-bold text-white group-hover:text-primary transition-colors"> ${job.jobTitle}</h3>
          </div>
          <p class="text-theme-muted leading-relaxed mb-8 flex-grow">${job.jobDescription} </p>
          <a href="./jobs/jobDetail.html?id=${job._id}"  class="inline-flex items-center gap-2 mt-10 text-primary text-xs font-bold uppercase tracking-widest hover:text-white transition-colors">
...
async function fetchLatestBlogs() {
    try {
        ...
        if (blogs.length === 0) {
            container.innerHTML = `
                <div class="swiper-slide p-2">
                    <div class="flex flex-col h-[580px] bg-section-light border border-pink-200 rounded-xl shadow-sm items-center justify-center">
                        <i class="fas fa-blog text-theme-muted text-6xl mb-4"></i>
                        <p class="text-theme-muted text-lg">No blogs available yet</p>
                        <p class="text-theme-muted text-sm">Check back soon for exciting content!</p>
                    </div>
                </div>
            `;
            return;
        ...
        blogs.forEach((blog) => {
            ...
            const cardHTML = `
                <div class="swiper-slide p-2">
                    <div class="flex flex-col h-[580px] bg-section-light border border-pink-200 rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden">
                        ...
                        <div class="flex flex-col flex-1 p-6">
                            ...
                            <h3 class="font-bold text-lg text-theme-heading mb-2 line-clamp-2">${blog.title}</h3>
                            <p class="text-theme-muted text-sm mb-4 flex-1 line-clamp-4">
                                ${excerpt}
                            </p>
                            <div class="mt-auto">
        ...
        container.innerHTML = `
            <div class="swiper-slide p-2">
                <div class="flex flex-col h-[580px] bg-section-light border border-pink-200 rounded-xl shadow-sm items-center justify-center">
                    ...
                    <p class="text-theme-muted text-sm">Please try again later</p>
                </div>
            </div>
        `;
    }
